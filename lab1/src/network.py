...
class CrossEntropyCost(object):
    ...
    def fn(a, y):
        """
        返回 a 和标签 y 之间的损失, 其中 np.nan_to_num 是用来确保数值稳定性, 
        比如 a 和 y 同时为 1.0 时, (1-y)*np.log(1-a)
...
class Network(object):
    ...
    def SGD(self,
        ...
        列表内存有每个 epoch 的计算结果。
        """
        evaluation_data, training_data = list(
        ...
        for j in range(epochs):
            ...
            for mini_batch in mini_batches:
                ...
                # if count % 500 == 0:
                #   print("epoch: {}, batch进度: {}/{}".format(j, count, batch_num))
                # count += 1
            ...
            if monitor_training_accuracy:
                accuracy_train = self.accuracy(training_data, convert=True) / n
            ...
            if monitor_evaluation_accuracy:
                accuracy_evalue = self.accuracy(evaluation_data) / n_data
                """
                if accuracy_tmp < accuracy_evalue:
                    eta = eta * abs(accuracy_evalue - accuracy_tmp) / 5
                elif abs(accuracy_evalue - accuracy_tmp) < (1 - accuracy_tmp):
                    eta = eta * abs(accuracy_evalue -
                                    accuracy_tmp) * 80 / pow(j, 2)
                accuracy_evalue = accuracy_tmp
                """
                evaluation_accuracy.append(accuracy_evalue)
                print("Accuracy on evaluation data: {} / {}\n".format(
                    accuracy_evalue * n_data, n_data))

        return evaluation_cost, evaluation_accuracy, training_cost, training_accuracy

    def update_mini_batch(self, mini_batch, eta, lmbda, n):
        """
        对单个 mini batch 使用梯度下降算法来更新网络的权重和偏置, 
        其中, mini_batch 是由元组 (x, y) 组成的列表, eta 是学习率, 
        ...
        for x, y in mini_batch:
            ...
            nabla_b = [n_b + d_n_b for n_b,
                       d_n_b in zip(nabla_b, delta_nabla_b)]
            nabla_w = [n_w + d_n_w for n_w,
                       d_n_w in zip(nabla_w, delta_nabla_w)]
        # tmps = self.weights
        self.weights = [
            (1 - eta * (lmbda / n)) * w - (eta / len(mini_batch)) * nw
            for w, nw in zip(self.weights, nabla_w)
        ]
        # print((tmp - x).min for tmp, x in zip(tmps, self.weights))
        self.biases = [
    ...
    def backprop(self, x, y):
        ...
        其中, nabla_b 和 nabla_w 是由 numpy arrays 组成的列表, 与
        ...
        for l in range(2, self.num_layers):
            ...
            delta = sp * np.dot(self.weights[-l + 1], delta)
    ...
    def accuracy(self, data, convert=False):
        """
        返回 data 中分类正确的图像的总数, 其中 convert 在训练集上应初始化为 True, 
        验证集或测试集上应初始化为 False 
        """
        if convert:
    ...
    def total_cost(self, data, lmbda, convert=False):
        """
        返回 data 对应的损失, 其中 convert 在训练集上应初始化为 False, 
        验证集或测试集上应初始化为 True, 
        `lmbda` 对应L2 正则化参数, 默认取值为 0.0, 即不进行 L2 正则化 
        """
        cost = 0.0
        for x, y in data:
...
def plot_result(epochs, test_cost, test_accuracy, training_cost,
                training_accuracy, file_name):
